...
<div class="flex flex-col bg-white rounded-lg shadow-md p-6">
    ...
    <form method="POST"
        action="<?php echo e(isset($items) ? route('dashboard.master.soal.update', ['soal' => $items->id]) : route('dashboard.master.soal.store')); ?>"
        class="space-y-6" x-data="{ tipe: '<?php echo e(old('tipe', $items->tipe ?? 'Pilihan ganda')); ?>' }">
        <?php if(isset($items)): ?>
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b pb-4">
            <div class="font-semibold text-xl text-gray-800"><?php echo e($action); ?> Soal</div>
        ...
        <div class="space-y-4">
            <div>
                ...
                <div x-data="trixEditor()" x-init="init()" class="bg-white rounded-xl overflow-hidden border border-gray-300 focus-within:border-[#177245] transition-colors">
                    <input id="nama_soal" type="hidden" name="nama" value="<?php echo e(old('nama', $items->nama ?? '')); ?>" x-ref="input">
...
unset($__errorArgs, $__bag); ?>
            ...
            <div x-show="tipe == 'Pilihan ganda'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 transform -translate-y-4" x-transition:enter-end="opacity-100 transform translate-y-0"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                    ...
                    <div class="group">
                        <label class="block text-gray-600 text-[10px] font-bold uppercase mb-1 ml-2 group-focus-within:text-green-600 transition-colors tracking-widest">Opsi <?php echo e(strtoupper($opt)); ?></label>
                        <div class="flex items-center gap-3">
                            <span class="flex items-center justify-center w-8 h-8 rounded-full bg-white border border-gray-300 font-bold text-gray-500 text-xs uppercase"><?php echo e($opt); ?></span>
                            <input type="text" name="opsi_<?php echo e($opt); ?>" :required="tipe == 'Pilihan ganda'" value="<?php echo e(old('opsi_'.$opt, $items->{'opsi_'.$opt} ?? '')); ?>"
...
unset($__errorArgs, $__bag); ?>
            ...
            <div class="p-5 bg-green-50/30 rounded-2xl border border-green-100 space-y-3">
                ...
                <div class="relative">
                    <input type="text" name="jawaban" required value="<?php echo e(old('jawaban', $items->jawaban ?? '')); ?>"
                        placeholder="<?php echo e($action == 'Tambah' ? 'Masukkan jawaban yang benar' : ''); ?>"
...
unset($__errorArgs, $__bag); ?>
        ...
        <div class="flex items-center justify-between pt-6 border-t">
            <a href="<?php echo e(route('dashboard.master.soal.index')); ?>" class="flex items-center gap-2 text-sm text-gray-500 hover:text-gray-800 transition-colors font-semibold group">
...
<?php echo $__env->make('base.layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\project\breskul\web\resources\views/master/soal/form.blade.php ENDPATH**/ ?>