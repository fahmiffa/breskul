...
<div class="flex flex-col bg-white rounded-lg shadow-md p-6" x-data="{
    ...
    <div class="mb-4 flex justify-between items-center gap-2">
        ...
        <div class="flex gap-2">
            <button @click="showTambahKelas = true" :disabled="selectedItems.length === 0"
                ...
                <?php echo e(config('app.school_mode') ? 'Kelas' : 'Prodi'); ?>
                        ...
                        <td class="px-4 py-2">
                            <div class="flex space-x-4 items-start">
                                ...
                                <div class="flex flex-col">
                                    <div class="font-semibold"><?php echo e(config('app.school_mode') ? 'Kelas' : 'Prodi'); ?></div>
                                    <template x-for="(val, index) in (row.kelas || row.prodi || [])" :key="index">
    ...
    <div x-show="showTambahKelas" x-cloak class="fixed inset-0 bg-black/50 flex items-center justify-center z-50"
        ...
        <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-md mx-5 md:mx-0"
            ...
            <h2 class="text-lg font-semibold mb-4">Set <?php echo e(config('app.school_mode') ? 'Kelas' : 'Prodi'); ?> untuk <span x-text="selectedItems.length"></span> <?php echo e(config('app.school_mode') ? 'Murid' : 'Mahasiswa'); ?>

            </h2>

            <form @submit.prevent="assignClass">
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Pilih <?php echo e(config('app.school_mode') ? 'Kelas' : 'Prodi'); ?></label>
                    <select x-model="selectedClass" required
                        ...
                        <?php else: ?>
                        <?php $__currentLoopData = \App\Models\Prodi::latest()->get(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $prodi): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <option value="<?php echo e($prodi->id); ?>"><?php echo e($prodi->name); ?></option>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        <?php endif; ?>
                    </select>
                </div>
    ...
    <div x-show="showJob" x-cloak class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" x-transition>
        <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-md mx-5 md:mx-0" @click.away="showJob = false">
            <h2 class="text-lg font-semibold mb-4">Import Semua <?php echo e(config('app.school_mode') ? 'murid' : 'mahasiswa'); ?> ke <?php echo e($akademik ? $akademik->name : null); ?></h2>
