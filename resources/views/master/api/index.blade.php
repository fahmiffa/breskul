...
@section('content')
    <div class="flex flex-col bg-white rounded-lg shadow-md p-6" x-data="dataTable({{ json_encode($items) }})">
        ...
        <div class="mb-4 flex justify-between items-center gap-2">
            ...
            <a href="{{ route('dashboard.master.api.create') }}"
            ...
            <table class="min-w-full bg-white border border-gray-200 text-sm">
                <thead>
                    <tr class="bg-green-500 text-left text-white">
                        ...
                        <th @click="sortBy('key')" class="cursor-pointer px-4 py-2">Key</th>
                ...
                <tbody>
                    <template x-for="(row, index) in paginatedData()" :key="row.id">
                        <tr class="border-t border-gray-300">
                            ...
                            <td class="px-4 py-2" x-text="row.key"></td>
                            <td class="px-4 py-2 flex items-center gap-1">
                                <form :action="'/dashboard/master/api/' + row.id" method="POST"
