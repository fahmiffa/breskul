...
<div class="flex flex-col bg-white rounded-lg shadow-md p-6">
    ...
    @isset($items)
    <form method="POST" action="{{ route('dashboard.master.fakultas.update', ['fakulta'=>$items->id]) }}" class="grid grid-cols-1">
        @method('PUT')
        @else
        <form method="POST" action="{{ route('dashboard.master.fakultas.store') }}" class="grid grid-cols-1">
            @endisset
            @csrf
            <div class="mb-4">
                <label class="block text-gray-700 text-sm font-semibold mb-2">Nama Fakultas</label>
                <div class="relative">
            ...
            <div class="mb-4">
                <label class="block text-gray-700 text-sm font-semibold mb-2">Kode Fakultas</label>
                <div class="relative">
