...
<div class="flex flex-col bg-white rounded-lg shadow-md p-6" x-data="{
                        ...
                        <td class="px-4 py-2">
                            <div class="flex space-x-4 items-start">
                                ...
                                <div class="flex flex-col">
                                    <div class="font-semibold">{{ config('app.school_mode') ? 'Kelas' : 'Prodi' }}</div>
    ...
    <div x-show="showTambahKelas" x-cloak class="fixed inset-0 bg-black/50 flex items-center justify-center z-50"
        ...
        <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-md mx-5 md:mx-0"
            ...
            <h2 class="text-lg font-semibold mb-4">Set {{ config('app.school_mode') ? 'Kelas' : 'Prodi' }} untuk <span x-text="selectedItems.length"></span> {{ config('app.school_mode') ? 'Murid' : 'Mahasiswa' }}
            </h2>

            <form @submit.prevent="assignClass">
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Pilih {{ config('app.school_mode') ? 'Kelas' : 'Prodi' }}</label>
                    <select x-model="selectedClass" required
                        ...
                        @foreach (\App\Models\Classes::latest()->get() as $class)
                        <option value="{{ $class->id }}">{{ $class->name }}</option>
                        @endforeach
                        @else
                        @foreach (\App\Models\Prodi::latest()->get() as $prodi)
    ...
    <div x-show="showJob" x-cloak class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" x-transition>
        <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-md mx-5 md:mx-0" @click.away="showJob = false">
            <h2 class="text-lg font-semibold mb-4">Import Semua {{ config('app.school_mode') ? 'murid' : 'mahasiswa' }} ke {{ $akademik ? $akademik->name : null }}</h2>
